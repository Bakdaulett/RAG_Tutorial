...
- **Change the Gemini model** – in `services/main.py`, update `MODEL_NAME` in `main()` / `RAGSystem`.
- **Adjust RAG retrieval behavior** – tweak `top_k` and `score_threshold` in `services/rag_generator.py`.
- **Change embedding model** – modify `Embedder`’s `model_name` in `embedding_manager.py`
  and ensure the corresponding Ollama model is pulled.
- **Change Qdrant connection** – update the URL in `qdrant_manager.QdrantManager`.
- **Routing behavior** – edit the routing prompt or document themes in `services/router_agent.py`.
...
- **Qdrant connection errors**:
  - Make sure the Qdrant container or service is running on `localhost:6333`.
  - Check firewall / port mappings.

- **Gemini authentication errors**:
  - Verify `GEMINI_API_KEY` in `.env`.
  - Confirm your key has access to the configured Gemini model.

---

## 9. License / usage

This repository is intended as a **tutorial / experimentation** project for RAG systems.
Adapt the code, prompts, and configurations to your own use cases and infrastructure.
